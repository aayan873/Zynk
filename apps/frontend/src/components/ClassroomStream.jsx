...
export default function ClassroomStream({ classroom, user, isTeacher, token }) {
    ...
    if (meets.length === 0) return (
        <div className="bg-[#14151a] rounded-2xl border border-dashed border-gray-800/80 p-8 sm:p-12 text-center flex flex-col items-center justify-center min-h-[12rem]">
            ...
            <p className="text-gray-500 text-xs sm:text-sm mt-1 max-w-xs">When the teacher schedules a new class, it will appear here.</p>
        </div>
    );
        ...
        <div className="space-y-3 sm:space-y-4 overflow-y-auto max-h-[60vh] custom-scrollbar pr-2">
            {meets.map((meet, index) => {
                ...
                return (
                    ...
                    >
                        {live && <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.8)]" />}
...
const Spinner = () => (
    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500/30 border-t-indigo-500" />
);